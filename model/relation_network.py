...
class RelationNetwork:
    def __init__(self, S, S_len, Q, Q_len, A, word2id, a_word2id, embed_size=32, lstm_size=32,
        ...
        self.embed_sz, self.lstm_sz, self.init = embed_size, lstm_size, initializer

        # Initialize Placeholders
        self.XS = tf.placeholder(tf.int32, shape=[None] + self.S.shape[1:], name='Story')
        self.XS_len = tf.placeholder(tf.int32, shape=[None] + self.S_len.shape[1:], name='Story_Length')
        self.XQ = tf.placeholder(tf.int32, shape=[None] + self.Q.shape[1:], name='Query')
        self.XQ_len = tf.placeholder(tf.int32, shape=[None], name='Query_Length')
        self.YA = tf.placeholder(tf.int32, shape=[None], name='Answer')
    ...
    def instantiate_weights(self):
        ...
        SE = tf.get_variable("Story_Embedding", [len(self.word2id), self.embed_sz], initializer=self.init)
        zero_mask = tf.constant([0 if i == 0 else 1 for i in range(len(self.word2id))],
                                dtype=tf.float32, shape=[len(self.word2id), 1])
        self.SE = SE * zero_mask

        # Create NL Embedding Matrix, with 0 Vector for PAD_ID (0) [Arg Net]
        QE = tf.get_variable("Query_Embedding", [len(self.word2id), self.embed_sz], initializer=self.init)
        self.QE = QE * zero_mask

        # Create Constant Tags, to Denote Relative Position
        tag_constant = np.zeros([self.story_len, 1], dtype=int)
        for i in range(self.story_len):
            tag_constant[i][0] = i
        self.tags = tag_constant

        # G_Theta Parameters
        self.G1_W = tf.get_variable("G1_W", shape=[2 + (3 * self.embed_sz), 256], initializer=self.init)
        ...
        self.F2_W = tf.get_variable("F1_W", shape=[256, 512], initializer=self.init)
        self.F2_b = tf.get_variable("F1_b", shape=[512], initializer=self.init)
    ...
    def inference(self):
        ...
        """
        # Encode Story Lines
        story_embeddings = tf.nn.embedding_lookup(self.SE, self.XS)                 # Shape: [None, s_len, max_s, emb]
        flat_embeddings = tf.reshape(story_embeddings,                              # Shape: [None * s_len, max_s, emb]
                                     shape=[-1, self.max_s, self.embed_sz])
        flat_lengths = tf.reshape(self.XS_len, [-1])                                # Shape: [None * story_len]

        with tf.variable_scope("S_Encoder"):
            self.story_gru = tf.contrib.gru(self.embed_sz)
            _, story_state = tf.nn.dynamic_rnn(self.story_gru, flat_embeddings,     # Shape: [None * story_len, emb]
                                               sequence_length=flat_lengths, dtype=tf.float32)
            s_obj = tf.reshape(story_state, shape=[-1, self.story_len,              # Shape: [None, story_len, emb]
                                                   self.embed_sz])
            s_tags = tf.tile(tf.expand_dims(self.tags, axis=1),
                             multiples=[tf.shape(s_obj)[0], 1, 1])                  # Shape: [None, story_len, 1]
            story_objects = tf.concat([s_tags, s_obj], axis=2)                      # Shape: [None, story_len, 1 + emb]

        # Encode Query
        query_embeddings = tf.nn.embedding_lookup(self.QE, self.XQ)                 # Shape: [None, max_q, emb]
        with tf.variable_scope("Q_Encoder"):
            self.query_gru = tf.contrib.gru(self.embed_sz)
        ...
        for i in range(self.story_len):
            for j in range(self.story_len):
                u = tf.squeeze(tf.concat([objects[i], objects[j], query], axis=2))  # Shape: [None, 2 + (3 * emb)]
                g = self.g_theta(u)
                sum_g_theta += g
        ...
        logits = tf.matmul(h2, self.F3_W) + self.F3_W
